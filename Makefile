...
#
# Tooling convention: every command goes through `uv run`, which auto-
# activates the project venv (created by `uv sync` in install-dev). No
# need to source .venv/bin/activate manually.
...
coverage:               ## Run pytest with coverage; fail under 80%.
	cd $(BACKEND) && uv run pytest --cov=api --cov=manage_users --cov-fail-under=80
